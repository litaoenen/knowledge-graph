...
const Graph3D: React.FC = () => {
  ...
  const addNode = () => {
    ...
    if (layoutType === 'manual' && !selectedPosition && !isSelectingPosition) {
      ...
      showToast('请在场景中点击选择节点位置', 'success');
      
      // 禁用相机控制，使点击事件可以用于选择位置
  ...
  // 完成添加节点（实际创建节点的逻辑）
  const finishAddNode = (nodeData: Partial<KnowledgeNode & AbilityNode>, position: THREE.Vector3 | null) => {
    // 使用typeScript提供的断言，确保id和label不为undefined
  ...
  const handleSceneClick = (event: any) => {
    ...
    // 获取点击位置
    const position = event.point ? new THREE.Vector3().copy(event.point) : new THREE.Vector3(0, 0, 0);
    
    // 保存选择的位置并完成节点添加
    setSelectedPosition(position);
    if (pendingNodeData) {
      finishAddNode(pendingNodeData, position);
  ...
  // 建立场景引用
  const setSceneRef = (ref: any) => {
    sceneRef.current = ref;
  };
  
  // 建立相机控制引用
  const setCameraControlsRef = (ref: any) => {
    cameraControlsRef.current = ref;
  };
  ...
  return (
    <div style={{ width: '100%', height: '80vh', position: 'relative' }}>
      ...
      <Canvas style={{ background: '#111' }}>
        <SafeScene 
          ref={setSceneRef}
          onClick={isSelectingPosition ? handleSceneClick : undefined}
        >
          <ambientLight intensity={0.5} />
          <pointLight position={[10, 10, 10]} />
          <OrbitControls ref={setCameraControlsRef} />
