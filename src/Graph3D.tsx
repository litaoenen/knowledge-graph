...
interface Edge3D {
  ...
  edgeType?: 'normal' | 'knowledge-ability'; // 添加边类型
}
...
const Graph3D: React.FC = () => {
  ...
  const [knowledgeAbilityConnections, setKnowledgeAbilityConnections] = useState<{ knowledge: string, ability: string }[]>([]);
  ...
  useEffect(() => {
    const loadExcelData = async () => {
      try {
        ...
        // 加载知识点-能力点映射数据
        try {
          const mappingResponse = await fetch('/knowledge_ability_mapping.xlsx');
          if (mappingResponse.ok) {
            const mappingArrayBuffer = await mappingResponse.arrayBuffer();
            const connections = await readKnowledgeAbilityMappingExcel(mappingArrayBuffer);
            if (connections && connections.length > 0) {
              console.log('从Excel映射文件成功加载了连接关系数据:', connections.length);
              setKnowledgeAbilityConnections(connections);
            }
          }
        } catch (mappingError) {
          console.warn('加载知识点-能力点映射数据失败:', mappingError);
          // 继续执行，不影响主要功能
        }
      } catch (error) {
  ...
  const createEdges = (nodes: Node3D[]): Edge3D[] => {
    ...
    // 添加知识点与能力点之间的连线
    if (knowledgeAbilityConnections.length > 0) {
      // 创建节点ID的查找映射，用于快速访问
      const nodeMap = new Map<string, Node3D>();
      nodes.forEach(node => nodeMap.set(node.id, node));
      
      knowledgeAbilityConnections.forEach(conn => {
        const knowledgeNode = nodeMap.get(conn.knowledge);
        const abilityNode = nodeMap.get(conn.ability);
        ...
        if (knowledgeNode && abilityNode) {
          edges.push({
            ...
            target: conn.ability,
            sourcePosition: knowledgeNode.position,
            targetPosition: abilityNode.position,
            edgeType: 'knowledge-ability'
          });
        }
      });
      
      console.log(`创建了 ${knowledgeAbilityConnections.length} 条知识点-能力点连线`);
    }
  ...
  return (
    <div style={{ width: '100%', height: '80vh', position: 'relative' }}>
      {loading ? (
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(to bottom, #1e293b, #0f172a)', color: 'white', borderRadius: '8px', fontSize: '18px' }}>
          <div className="loading-spinner"></div>
          <span style={{ marginLeft: '12px' }}>加载中...</span>
        </div>
      ) : error ? (
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(to bottom, #1e293b, #0f172a)', color: '#ef4444', borderRadius: '8px', fontSize: '18px' }}>
