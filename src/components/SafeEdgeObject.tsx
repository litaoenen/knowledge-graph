...
interface Edge3D {
  ...
  edgeType?: 'normal' | 'knowledge-ability'; // 添加边类型字段
}
...
const SafeEdgeObject: React.FC<{
  ...
  isSelected?: boolean;
  edgeType?: 'normal' | 'knowledge-ability'; // 添加边类型属性
}> = ({ source, target, sourceType, isSelected = false, edgeType = 'normal' }) => {
  ...
  try {
    ...
    const isDifferentPlane = Math.abs(start.z - end.z) > 2;
    ...
    let lineColor;
    if (isSelected) {
      lineColor = "#ffcc00"; // 选中状态颜色
    } else if (edgeType === 'knowledge-ability') {
      lineColor = "#ff4500"; // 知识点-能力点连线颜色（橙红色）
    } else {
      lineColor = sourceType === 'knowledge' ? "#3a86ff" : "#00b894"; // 普通连线颜色（知识/能力）
    }
    
    // 如果是知识点与能力点之间的连线或不同平面的节点，使用曲线连接
    if (edgeType === 'knowledge-ability' || isDifferentPlane) {
      // 构建贝塞尔曲线路径
      ...
      // 设置知识点-能力点连线的特殊样式
      const dashSize = edgeType === 'knowledge-ability' ? 0.3 : 0; // 只有知识点-能力点连线使用虚线
      const gapSize = edgeType === 'knowledge-ability' ? 0.2 : 0;
        ...
        <group>
          <primitive object={new THREE.Line(
            ...
            new THREE.LineDashedMaterial({ 
              color: lineColor,
              linewidth: edgeType === 'knowledge-ability' ? 3 : 2,
              dashSize: dashSize,
              gapSize: gapSize,
              opacity: edgeType === 'knowledge-ability' ? 1 : 0.8,
              transparent: true
            })
          )} />
